...
public class IssueBook extends JFrame implements ActionListener{
	...
	JLabel l8 = new JLabel("Member_id");
	...
	panel_1.setBorder(new TitledBorder(new LineBorder(new Color(70, 130, 180), 2, true), "Member-Deatails",
    ...
    public void actionPerformed(ActionEvent ae){
		...
		rs.close();
            ...
            if(ae.getSource() == b3){
                if (bflag.equals("j")&&mflag.equals("s"))
                        {
                            JOptionPane.showMessageDialog(null, "Journals can't be issued to students.");
                        }
                else{
                    try{
                ...
                String sql = "insert into issueBook(book_id, member_id, bname, mname, course, branch, dateOfIssue, returnBy, mflag) values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
		PreparedStatement st = con.c.prepareStatement(sql);
		st.setString(1, t1.getText());
		...
		st.setString(7, ((JTextField) dateChooser.getDateEditor().getUiComponent()).getText());
                st.setString(8, ((JTextField) dateChooser2.getDateEditor().getUiComponent()).getText());
                st.setString(9, mflag);
